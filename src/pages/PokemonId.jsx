  ...
  return (
    <main>
      ...
      <section className='grid gap-20 justify-items-center pt-28 pb-28'>
        <article className={`w-[290px] xxs:w-[350px] xs:w-[390px] sm:w-[550px] md:w-[650px] lg:w-[690px] border-4 rounded-lg ${borderColors[pokemon?.types[0].type.name]}`}>
          ...
          <section className='px-2'>
            <article className='grid justify-items-center gap-3'>
              <h5 className='mt-14 font-semibold text-lg sm:text-xl'># {pokemon?.id}</h5>
              <h3 className={`text-3xl md:text-4xl font-semibold ${textColor[pokemon?.types[0].type.name]}`}>{pokemon?.name}</h3>
              <div className='h-1 bg-gray-400/30 min-w-[90%]'></div>
              <div className='grid grid-cols-2 gap-4'>
                <div className='text-center'>
                  <h5 className='text-center font-semibold text-base'>Weight</h5>
                  <span>{pokemon?.weight}</span>
                </div>
                <div className='text-center'>
                  <h5 className='text-center font-semibold text-base'>Heigth</h5>
                  ...
                  <div className='flex justify-center gap-x-7'>
                    {
                      pokemon?.types.map((type) => (
                        <span className={`px-5 py-1 rounded-md ${bgColor[pokemon?.types[0].type.name]}`} key={type.type.url}>{type.type.name}</span>
                      ))
                    }
                  </div>
                  ...
                  <div className='flex justify-center gap-x-8'>
                    {
                      pokemon?.abilities.map((ability) => (
                        <span className='px-5 py-1 rounded-md' key={ability.ability.url}>{ability.ability.name}</span>
                      ))
                    }
                ...
                pokemon?.stats.map((stat) => (
                  <article key={stat.stat.url}>
                    ...
                    <div className='bg-gray-300 h-8 rounded-md overflow-hidden'>
                      <div style={{ width: percenProgresStat(stat.base_stat) }} className={' h-full bg-gradient-to-r from-yellow-200 to-yellow-600'}></div>
                    </div>
        ...
        <section className='w-[290px] xxs:w-[350px] xs:w-[390px] sm:w-[550px] md:w-[650px] lg:w-[690px] border-4 rounded-lg'>
          <article className='p-2 flex flex-wrap gap-4'>
            ...
            {pokemon?.moves.map((move) => (
              <span className='text-base px-2 py-1 bg-gray-300 rounded-md' key={move?.move.url}>{move?.move.name}</span>
            ))}
          </article>
        </section>
